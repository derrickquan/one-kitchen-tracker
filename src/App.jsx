...
// MODIFIED: Updated auth imports for Email/Password
import { 
    getAuth, 
    onAuthStateChanged,
...
const PlusIcon = () => (
  <svg xmlns="http://www.w.org/2000/svg" width="20" height="20" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2" strokeLinecap="round" strokeLinejoin="round"><path d="M5 12h14"/><path d="M12 5v14"/></svg>
);
const EditIcon = () => (
  <svg xmlns="http://www.w.org/2000/svg" width="16" height="16" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2" strokeLinecap="round" strokeLinejoin="round"><path d="M17 3a2.828 2.828 0 1 1 4 4L7.5 20.5 2 22l1.5-5.5L17 3z"></path></svg>
);
const DeleteIcon = () => (
...
function App() {
    // MODIFIED: State management now includes user, loading, and auth error
    ...
    const appId = typeof __app_id !== 'undefined' ? __app_id : 'one-kitchen-tracker-prod';
    
    // MODIFIED: This useEffect now initializes Firebase and sets up the auth state listener
    useEffect(() => {
        try {
    ...
    // MODIFIED: Data fetching useEffect now depends on the `user` object
    useEffect(() => {
        if (!db || !user) {
        ...
        const userId = user.uid; // Get userId from the user object
    ...
    }, [db, user, appId]); // Reruns when user logs in or out
    ...
    // Your original application UI is returned here when a user is logged in
    return (
            ...
            <header className="bg-gray-800/50 backdrop-blur-sm sticky top-0 z-40 border-b border-gray-700">
                <nav className="container mx-auto px-4 sm:px-6 lg:px-8">
                    <div className="flex items-center justify-between h-16">
                        ...
                        {/* NEW: Logout Button and User Email */}
...
// ... THE REST OF YOUR COMPONENTS (Dashboard, CrudView, StatementUploadModal, etc.) remain unchanged below ...
...
function CrudView({ title, data, db, userId, appId, collectionName, fields, formatCurrency, vendors }) {
    ...
    return (
        <div className="bg-gray-800 p-6 rounded-lg border border-gray-700">
            ...
            {/* Data Table */}
            <div className="overflow-x-auto">
                 {collectionName === 'expenses' && groupedData.groups && groupedData.sortedCategories ? (
                    <div className="space-y-6">
                        {groupedData.sortedCategories.map(groupKey => (
                            <div key={groupKey}>
                                <div onClick={() => toggleGroup(groupKey)} className="flex justify-between items-center bg-gray-700/50 p-3 rounded-t-lg cursor-pointer">
                                    ...
                                    <div className="flex items-center gap-4">
                                         <span className="font-bold text-indigo-400">{formatCurrency(groupedData.groups[groupKey].total)}</span>
                                         <ChevronDownIcon />
                                    </div>
                                </div>
                                {expandedGroups[groupKey] && (
                                     <div className="overflow-x-auto border-x border-b border-gray-700/50 rounded-b-lg">
                                        {renderTable(groupedData.groups[groupKey].items)}
                                     </div>
                                )}
                            </div>
                        ))}
